/**
 * ============================================================
 * sensorhandle_task.c — 传感器数据处理任务
 * ============================================================
 * 功能:
 *   使用 FreeRTOS 队列集 (Queue Set) 统一管理多路传感器数据:
 *   1. STP23L 激光测距模块 → 解析后更新全局高度值
 *   2. STL06N 360°激光雷达 → 点云数据处理
 *
 * 设计要点:
 *   - union 共用体节省内存 (两路传感器数据不同时使用)
 *   - Queue Set 阻塞等待, 任一队列有数据即唤醒处理
 *   - STP23L 帧长度195字节为完整一帧
 * ============================================================
 */
