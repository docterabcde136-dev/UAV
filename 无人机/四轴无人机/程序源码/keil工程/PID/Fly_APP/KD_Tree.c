/**
 * ============================================================
 * KD_Tree.c — 二维KD-Tree + ICP点云配准定位算法
 * ============================================================
 * 算法概述:
 *   使用 2D KD-Tree 加速 ICP (Iterative Closest Point) 配准,
 *   通过激光雷达连续两帧点云数据计算飞行器的相对位移。
 *
 * 核心流程:
 *   1. 极坐标→直角坐标转换 (三角函数查表法, 0.3°分辨率)
 *   2. 第1-3帧: 建立模板KD-Tree (去中心化后)
 *   3. 第4帧起: 当前点云与模板匹配
 *      - 查找每个点的最近邻 (曼哈顿距离)
 *      - 用中位数估计整体偏移
 *      - 迭代至偏移收敛 (<0.1mm) 或最大50次
 *   4. 累计偏移>50mm时更新模板
 *
 * 性能优化:
 *   - 静态数组分配 (避免malloc)
 *   - 三角函数查表 (1200个条目, 分辨率0.3°)
 *   - 快速排序替代插入排序 (O(nlogn) vs O(n²))
 * ============================================================
 */
