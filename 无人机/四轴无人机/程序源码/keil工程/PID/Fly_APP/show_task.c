/**
 * ============================================================
 * show_task.c — OLED显示/APP数据传输任务
 * ============================================================
 * 功能:
 *   1.  APP模式:  通过蓝牙串口分时发送3页数据
 *      - 第1页: 码盘值 + 电量百分比 + 偏航角
 *      - 第2页: 横滚/俯仰/偏航角 (波形显示)
 *      - 第3页: 电流 + 电池电压 + 光流状态
 *   2. 上位机模式:  通过DataScope协议发送10通道波形数据
 *
 * 数据来源: ADC (电流/电压), appshow 全局结构体 (姿态数据)
 * ============================================================
 */
...
/**
 * @brief 显示任务 — 分时发送遥测数据到APP或上位机
 * @note  50ms周期, 分3帧轮询发送 (每帧150ms刷新一次)
 */
