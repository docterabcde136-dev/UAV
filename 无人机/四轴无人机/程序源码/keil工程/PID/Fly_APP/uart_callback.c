/**
 * ============================================================
 * uart_callback.c — UART中断回调处理
 * ============================================================
 * 功能:
 *   1. HAL_UART_RxCpltCallback — 单字节接收完成中断
 *      - huart4 (蓝牙): 软件复位检测 + 写入APP控制队列
 *      - huart1 (调试串口): 软件复位检测
 *   2. HAL_UARTEx_RxEventCallback — DMA不定长接收完成中断
 *      - huart5 (STP23L): 数据写入队列, 重新使能DMA
 *      - huart2 (STL06N): 数据写入队列, 重新使能DMA
 *      - huart3 (LC307): 光流模块数据处理
 *
 * 注意: 所有操作在中断上下文中执行, 队列操作用 FromISR 版本
 * ============================================================
 */
