/**
 * ============================================================
 * bluetoothControl_task.c — 蓝牙APP控制任务
 * ============================================================
 * 功能:
 *   1. 接收 WHEELTEC APP 蓝牙指令 (虚拟摇杆/按键/重力球)
 *   2. 解析 APP 自定义协议帧 ({M:1} 启动, {M:2} 停止等)
 *   3. 支持多页面: 摇杆页(1), 按键页(2), 重力球页(0, 已禁用)
 *   4. 接收 APP 参数调节指令 ({0:xxx} ~ {8:xxx})
 *
 * 数据流: 蓝牙→UART4→DMA→队列→本任务→控制队列→平衡控制任务
 * ============================================================
 */
