...
/* Private define ------------------------------------------------------------*/
/* USER CODE BEGIN PD */
#define userconfig_OPEN_STACK_CHECK 0     //开启任务堆栈大小打印
#define userconfig_OPEN_CPU_USAGE_CHECK 0 //开启任务CPU占用率打印
#define userconfig_OPEN_CHECK_HEAPSIZE 0  //打印剩余的堆大小
/* USER CODE END PD */
...
TaskHandle_t g_xTaskHandleCoordTask;
    ...
    //队列句柄
    QueueHandle_t g_xQueuestp23L_Ori;     //外围设备：stp23L雷达 原始数据的读取
    QueueHandle_t g_xQueueN10_Ori;        //外围设备：N10雷达        原始数据的读取
QueueHandle_t g_xQueuestl06n_Ori;

    QueueSetHandle_t g_xQueueSetSensor;   //传感器消息队列集合，汇聚所有传感器的数据
    QueueHandle_t g_xQueueFlyControl;     //飞行器控制队列
    QueueHandle_t g_xQueueBlueTooth_Ori;  //蓝牙APP原始数据
    QueueHandle_t g_xQueueLidarBuffer;    //雷达数据队列

    //事件组
    EventGroupHandle_t g_xEventFlyAction; //飞行器事件组句柄
...
__weak unsigned long getRunTimeCounterValue(void)
{
  ...
  static uint16_t nowtime = 0;
      ...
      //若本次计数值小于上次计数值，说明发生了定时器溢出
  if( nowtime < lasttime )
  {
        time += (nowtime + 0xffff - lasttime); //累加定时器周期
  }		
      else time += ( nowtime - lasttime ) ; //未溢出则直接为当前时间-上次时间
...
void MX_FREERTOS_Init(void) {
  ...
  /* add events, ... */
      g_xEventFlyAction = xEventGroupCreate();//创建飞行器事件组句柄的实现
...
void StartDefaultTask(void *argument)
{
  ...
  for(;;)
  {
        /* 等待所有初始化完成后再使能各种外设的中断，确保每个数据都能成功被系统接收 */
    vTaskDelay(400);
