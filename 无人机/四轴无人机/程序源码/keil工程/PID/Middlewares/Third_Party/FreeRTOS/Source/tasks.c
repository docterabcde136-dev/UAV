...
/* 将 configUSE_STATS_FORMATTING_FUNCTIONS 设置为 2 以包含统计格式化
函数但不在此处包含 stdio.h。 */
#if ( configUSE_STATS_FORMATTING_FUNCTIONS == 1 )
	/* 在此文件底部有两个可选函数，可用于
从 uxTaskGetSystemState() 函数生成的原始数据生成
人类可读文本。注意格式化函数仅为方便提供，
不被视为内核的一部分。 */
	#include <stdio.h>
#endif /* configUSE_STATS_FORMATTING_FUNCTIONS == 1 ) */

#if( configUSE_PREEMPTION == 0 )
	/* 如果使用协作式调度器，则不应仅因为更高优先级的任务被唤醒就执行让出。 */
...
/* 如果设置了以下任何一项，则用已知值填充任务栈，
以便确定高水位线。如果未设置以下任何一项，
则不填充栈，以免对 memset 产生不必要的依赖。 */
 ...
 */
typedef struct tskTaskControlBlock 			/* 使用旧的命名约定以防止破坏内核感知调试器。 */
{
