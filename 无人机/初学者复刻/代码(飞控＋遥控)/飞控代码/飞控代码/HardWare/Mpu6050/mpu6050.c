#include "MPU6050.h"   //CHUANSHONGMEN
...
{ 
	...
	MPU_Set_Gyro_Fsr(3);					//陀螺仪传感器,±2000dps    //CHUANSHONGMEN原来是3
	...
	if(res==MPU_ADDR)//器件ID正确  //if(res==MPU_ADDR)  CHUANSHONGMEN
	{
...
u8 MPU_Get_Gyroscope(float *gx,float *gy,float *gz)
{
  u8 buf[6],res;  
	...
	if(res==0)
	{
		gx_raw=((u16)buf[0]<<8)|buf[1];  
		gy_raw=((u16)buf[2]<<8)|buf[3];  
		gz_raw=((u16)buf[4]<<8)|buf[5];
